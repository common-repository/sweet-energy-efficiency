...
if ( ! function_exists( 'see_is_single_or_archive_book' ) ):
	function see_is_single_or_archive_book() {
		return
			(
				is_singular( 'book' )
				||
...
if ( ! function_exists( 'see_is_archive_book' ) ):
	function see_is_archive_book() {
		return
			(
...
if ( ! function_exists( 'see_get_template_loader' ) ):
	function see_get_template_loader() {
		return Winter_Activity_Log_Global::template_loader();
	}
endif;


if ( ! function_exists( 'see_get_column_class' ) ):
	/**
	 * @param $int int
	 *
	 * @return $css_class string
	 */
	function see_get_column_class( $int ) {
...
if ( ! function_exists( 'see_sanitize_color' ) ):
	function see_sanitize_color( $value ) {
...
if ( ! function_exists( 'see_get_the_user_ip' ) ):
	function see_get_the_user_ip() {
...
if ( ! function_exists( 'see_get_uri' ) ):
	function see_get_uri($skip_query_string = FALSE)
	{
...
if ( ! function_exists('see_resolve_level'))
{
	function see_resolve_level($request_uri = NULL, $post = NULL, $get = NULL, $request_method = NULL, $body = NULL, $fixed_level = NULL)
    {
		...
		{
			$request_uri = see_get_uri();
			$post = wmvc_xss_clean($_POST);
			$get = wmvc_xss_clean($_GET);
		...
		if(	
			...
			strpos($request_uri, 'wp_scrape_nonce') === FALSE &&
		...
		if(	isset($body['edit_date']) || 
			isset($body['post_type']) || 
			isset($body['post_ID']) || 
			isset($body['post_status']) || 
			isset($body['content']) || 
			isset($post['post_type']) ||
			isset($post['post_ID']) ||
			isset($post['post_status']) ||
			isset($post['edit_date']) ||
			isset($post['content']) )
		{
			$level = 4;
		}
...
if ( ! function_exists('see_generate_label_by_level'))
{
	function see_generate_label_by_level($level, $message = '')
	{
		if(empty($message))$message = $level;

		if($level==1)
		{
			return '<span class="label label-default" title="'.__('Most basic activities log, like when someone open some page','sweet-energy-efficiency').'">'.$message.'</span>';
		}
		if($level==2)
		{
			return '<span class="label label-primary" title="'.__('Something is sent in POST via ajax, sometimes this mean change in database','sweet-energy-efficiency').'">'.$message.'</span>';
		}
		if($level==3)
		{
			return '<span class="label label-success" title="'.__('Something general is sent in POST to regular page, mostly this mean change in database','sweet-energy-efficiency').'">'.$message.'</span>';
		}
		if($level==4)
		{
			return '<span class="label label-info" title="'.__('Editing known contents like post, page or similar','sweet-energy-efficiency').'">'.$message.'</span>';
		}
		if($level==5)
		{
			return '<span class="label label-warning" title="'.__('Critical tasks like FAILED login','sweet-energy-efficiency').'">'.$message.'</span>';
		}
		if($level==6)
		{
			return '<span class="label label-danger" title="'.__('Very critical tasks like hacking','sweet-energy-efficiency').'">'.$message.'</span>';
		}
...
if ( ! function_exists('see_resolve_wp_menu'))
{
    function see_resolve_wp_menu($page=NULL, $request_uri=NULL)
    {
		...
		if(!isset($submenu_c))
		{
			$menu_c = get_option( 'sweet-energy-efficiency-menuitems' );
			$submenu_c = get_option( 'sweet-energy-efficiency-submenuitems' );
		}
...
function see_generate_description($row, $failed_login = false)
{
	$desc = '';

	$desc .= see_resolve_wp_menu($row['page'], $row['request_uri']);
...
if ( ! function_exists('see_visitor_type'))
{
    function see_visitor_type($page=NULL, $request_uri=NULL, $user_agent = NULL)
    {
		if(empty($page) && empty($request_uri))return NULL;
...
function see_user_info($user_id, $header_data = array(), $page=NULL, $request_uri=NULL)
{
	...
	elseif(see_visitor_type($page, $request_uri, $user_agent ) === 'system')
	{
		$text = 'System';
	}
	elseif(see_visitor_type($page, $request_uri, $user_agent ) === 'guest')
	{
		$text = 'Guest';

	}
	elseif(see_visitor_type($page, $request_uri, $user_agent ) === 'unknown')
	{
		$text = 'Not logged in';
	}
...
if ( ! function_exists('see_prepare_search_query_GET'))
{
	function see_prepare_search_query_GET($columns = array(), $model_name = NULL, $external_columns = array())
	{
		$CI = &see_get_instance();
...
function see_send_remote_syslog($PAPERTRAIL_HOSTNAME, $PAPERTRAIL_PORT, $message, $component = "web", $program = "next_big_thing")
{
...
function see_remotedb_log($row_cloud, $log_message, $insert_array)
{
	global $wpdb;
...
function see_access_allowed($cap)
{
	$allowed_admins = get_option('see_allowed_admins');
	...
	$allowed_roles = get_option('see_allowed_roles');
...
function see_send_sms($phone_number, $message)
{
	if(!empty(get_option('see_clickatell_one_api_key')))
	{
		see_send_sms_one($phone_number, $message);
	}
	else if(!empty(get_option('see_clickatell_http_api_key')))
	{
		see_send_sms_http($phone_number, $message);
	}
	else if(!empty(get_option('see_smsapicom_http_api_key')))
	{
		see_sms_send_smsapicom($phone_number, $message);
	}
	else if(!empty(get_option('see_smsto_api_key')))
	{
		see_smsto($phone_number, $message);
	}
}
...
function see_wp_authenticate( $username, $password ) {
